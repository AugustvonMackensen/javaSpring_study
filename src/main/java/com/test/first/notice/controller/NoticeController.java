...
public class NoticeController {
	...
	public String moveUpdatePage(@RequestParam int noticeno, Model model) {
	...
	public String noticeDetailMethod(
		...
		//관리자용 상세보기 페이지와 일반 회원 상세보기 페이지 구분
	...
	public String noticeDeleteMethod(@RequestParam("noticeno") int noticeno, @RequestParam(name="rfile", required=false) String renameFileName,
		...
		if(noticeService.deleteNotice(noticeno) > 0) {
			...
			model.addAttribute("message", noticeno + "빈 공지 삭제 실패!");
			return "common/error";
		}
	}
	...
	public String noticeUpdateMethod(Notice notice, Model model, @RequestParam(name="delFlag", required=false) String delFlag,
